...
let prefix = await require('quick.db').fetch(`prefix_${message.guild.id}`) || ayarlar.prefix
  ...
  if (aktif) {
    ...
    message.channel.send(`:white_check_mark:> Bot eklendiğinde atılması için ayarlanmış sistem başarıyla kapatıldı. `)
  }
 
  if (!aktif) {
    db.set(`bottemizle_${message.guild.id}`, 'aktif')
    message.channel.send(`:white_check_mark: Başarılı! Artık sunucuya bot eklendiğinde otomatikmen güvenlik sebebiyle atılacak. `)
  }
};
